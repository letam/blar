...
1. **Make the posts repo** (public, so the page is readable):
   ```
   gh repo create letam/blar-posts --public --clone
...
3. **Sanity-check the path works before building anything else:**
   ...
   curl -X PUT -H "Authorization: Bearer YOUR_TOKEN" \
     ...
     https://api.github.com/repos/letam/blar-posts/contents/posts/test.md \
...
## deploy the app
```
gh repo create letam/blar --public --clone   # the app (separate from blar-posts, the content)
# copy index.html in
git add -A && git commit -m "blar v0" && git push
# github → Settings → Pages → main / root → enable  →  https://letam.github.io/blar
```
...
## shipped =
**3 of your own posts**, real and unedited, publicly readable at `github.com/letam/blar-posts/tree/main/posts`, each committed via the publish button.
